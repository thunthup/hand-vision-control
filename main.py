  ...
  while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
      for hand_landmarks in results.multi_hand_landmarks[:1]:
        ...
        min_x = int(min([hand_landmarks.landmark[e].x for e in range(21)]) * width) - 20
        max_x = int(max([hand_landmarks.landmark[e].x for e in range(21)]) * width) + 20
        min_y = int(min([hand_landmarks.landmark[e].y for e in range(21)]) * height) - 20
        max_y = int(max([hand_landmarks.landmark[e].y for e in range(21)]) * height) + 20
        last_hold = hold
