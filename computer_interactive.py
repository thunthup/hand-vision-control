...
def detect_click(hand_landmarks, cutoff = 0.6) :
    x = [hand_landmarks.landmark[e].x for e in range(21)]
    y = [hand_landmarks.landmark[e].y for e in range(21)]
    height = max(y) - min(y)
    left_distance = euclidean_distance((x[0], y[0]), (x[8], y[8]))/height
    right_distance = euclidean_distance((x[0], y[0]), (x[12], y[12]))/height
    print(left_distance, right_distance)
    left_click = left_distance <= cutoff + 0.1
    right_click = right_distance <= cutoff
    no_click = not left_click and not right_click
    return no_click, left_click, right_click
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks[:1]:
                ...
                no_click, left_click, right_click = detect_click(hand_landmarks)
                ...
                if len(center) >= 2:
                    ...
                    move_x = (last_point[0] - current_point[0]) * 2
                    move_y = (current_point[1] - last_point[1]) * 2
                    pyautogui.move(move_x, move_y)

                if left_click and not last_left_click :
                    pyautogui.mouseDown()
                elif not left_click and last_left_click :
                    pyautogui.mouseUp()
                
                if right_click and not last_right_click :
                    pyautogui.mouseDown(button = "right")
                elif not right_click and last_right_click :
